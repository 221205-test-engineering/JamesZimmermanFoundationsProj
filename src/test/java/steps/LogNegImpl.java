...
public class LogNegImpl{
    ...
    public void the_employee_should_see_an_alert_saying_they_have_the_wrong_password() {
        ...
        String wP = driver.switchTo().alert().getText();

        Assert.assertEquals("Wrong password for User",wP);
    ...
    public void the_employee_types_in_natchamps_into_the_password_input(){
       WebElement pass = (WebElement) new WebDriverWait(driver,Duration.ofSeconds(5))
                .until(ExpectedConditions.elementToBeClickable(foundNeg.sub));
    ...
    public void the_employee_should_see_an_alert_saying_no_user_with_that_username_found(){
        ...
        String wU = driver.switchTo().alert().getText();

        Assert.assertEquals("Username not found",wU);
